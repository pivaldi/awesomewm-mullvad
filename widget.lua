...
local wdg = wibox.widget {
  ...
  connect = function(self)
    ...
    awful.spawn.easy_async(
      cli_path .. " connect " .. connect_options,
...
local function init()
  if not is_initialized then
    ...
    local mv_timer = timer({ timeout = timeout })
    mv_timer:connect_signal(
      "timeout",
      function()
        update_status()
      end)
...
local function worker(user_args)
  ...
  cli_path = args.cli_path or cli_path
